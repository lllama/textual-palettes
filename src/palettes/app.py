...
class PaletteInfo:
    ...
    def from_json(cls, args):
        return cls(**{key.name: args[key.name] for key in fields(cls)})
...
MAX_COLOURS = 12
PALETTES_URL = "https://lospec.com/palette-list/load?colorNumberFilterType=max&colorNumber={MAX_COLOURS}&page={PAGE}&tag=&sortingType=default"
...
class Palette(Container):
    ...
    palette = reactive(
        PaletteInfo(
            tags=[],
            title="default",
            description="Textual default palette",
            slug="",
            colors=[],
        )
    )
    design = reactive({}, layout=True)
    ...
    def watch_design(self, new_value):
        ...
        for key in self.colour_vars:
            try:
                colour = self.query_one(f"#{key}")
                colour.styles.background = colours[key]
                colour.styles.color = colour.styles.background.get_contrast_text()
            except NoMatches:
                pass
    ...
    def compose(self) -> ComposeResult:
        ...
        for key in self.colour_vars:
            ...
            color.styles.background = colours[key]
            color.styles.color = color.styles.background.get_contrast_text()
            yield color


class DemoApp(App):
    CSS_PATH = "app.css"
    TITLE = "Textual Palettes"
    BINDINGS = [
        ("space", "next_palette", "Next Palette"),
        ("b", "previous_palette", "Previous Palette"),
        ("c", "cycle_palette", "Cycle Palette"),
    ...
    async def get_palettes(self):
        try:
            while response := await self.client.get(
                PALETTES_URL.format(MAX_COLOURS=MAX_COLOURS, PAGE=self.page)
            ):
                self.page += 1
                self.palettes.extend(
                    PaletteInfo.from_json(palette)
                    for palette in response.json()["palettes"]
                )
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Container(
            ...
            Body(
                ...
                Column(
                    Section(Welcome(), classes="location-first"),
                ),
                Column(
                    Section(
